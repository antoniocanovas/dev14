...
#     @api.multi
    def on_update_message(self, message):
        """ Update the message for pos multi session sync """ 
        self.ensure_one()
...
#     @api.multi
    def prepare_new_session(self, message):
        """ Update the state for old orders """
        self.ensure_one()
...
#     @api.multi
    def check_order_revision(self, message, order):
        """ Check the order revision and show the warning message """
        self.ensure_one()
...
#     @api.multi
    def set_changes(self, message, order):
        """ Set Changes in the multi session sync """
        self.ensure_one()
...
#     @api.multi
    def set_and_broadcast_order(self, message):
        """ Update revision id and message to the pos multi sync order """
        self.ensure_one()
...
#     @api.multi
    def set_order(self, message):
        """ Update the order id, message, run_id and revision_id """
        self.ensure_one()
...
#     @api.multi
    def get_sync_all(self, message):
        """ Get the order details to sync the order with pos """
        self.ensure_one()
...
#     @api.multi
    def remove_order(self, message):
        """ Remove the order and update the state as deleted  """
        self.ensure_one()
        order_uid = message['data']['uid']
...
#     @api.multi
    def send_sync_message(self, message):
        """ Send the sync message pos multi session """
        self.ensure_one()
        notifications = []
...
#     @api.multi
    def broadcast_message(self, message):
        """ Update message to the pos multi session sync """
        self.ensure_one()
        notifications = []
        channel_name = "pos.multi_session"
