...
class MailMessage(models.Model):
    ...
    def get_mail_message_name(self):
        for record in self:
            ...
            if (model_id.id and model_id.name): name += model_id.name
            if (model_id.id and model_id.name and item_id.id and item_id.name): name += " => "
            if (item_id.id and item_id.name): name += item_id.name
            record['name'] = name
